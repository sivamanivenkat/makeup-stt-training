...
async function phasePackage() {
  ...
  log.info(`${cleanedRaw.length} raw cleaned entries, ${cleaned.length} unique segments`);
  ...
  for (const [split, entries] of Object.entries(splits)) {
    ...
    for (const entry of entries) {
      ...
      const srcPath = path.join(process.env.AUDIO_DIR || './temp/audio', 'segments', destName);
