...
function main() {
  ...
  if (!badIds.size) {
    log.success("No corruption found");
    return;
  }
  for (const id of badIds) log.warn(`  ${id}`);

  const dl = filterCheckpoint(
    path.join(CKPT_DIR, "downloaded.jsonl"),
    "segmentId",
    badIds,
  );
  const tr = filterCheckpoint(
    path.join(CKPT_DIR, "transcribed.jsonl"),
    "segment_id",
    badIds,
  );
  const cl = filterCheckpoint(
    path.join(CKPT_DIR, "cleaned.jsonl"),
    "segment_id",
    badIds,
  );
  ...
  log.stat(
    "Next step",
    "node orchestrator.js download   (re-extracts these cleanly)",
  );
}
