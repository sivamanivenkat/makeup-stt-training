...
function checkSplit(split) {
  ...
  let ok = 0, missingAudio = 0, emptyText = 0;
  ...
  for (const line of lines) {
    ...
    if (!hasAudio) { missingAudio++; log.warn(`Missing audio: ${audioPath}`); }
    if (!hasText)  { emptyText++;   log.warn(`Empty transcript: ${entry.segment_id}`); }
...
for (const split of ['train', 'val', 'test']) {
  ...
  grandTotal += r.total;
  grandOk   += r.ok;
  log.stat(split, `${r.ok}/${r.total} OK — ${r.missingAudio} missing audio, ${r.emptyText} empty text`);
}
...
if (fs.existsSync(failedPath)) {
  const failed = fs.readFileSync(failedPath, 'utf8').trim().split('\n').filter(Boolean).length;
...
if (grandOk === grandTotal) {
  ...
  log.warn(`${grandTotal - grandOk} segments have issues — check warnings above`);
}
