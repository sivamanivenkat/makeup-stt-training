...
def transcribe_segment(model, align_model, align_metadata, entry, device, batch_size):
    ...
    words = [
        {"word": w["word"], "start": w.get("start", 0), "end": w.get("end", 0), "score": w.get("score", 0)}
...
def main():
    ...
    print(f"[transcriber] device={args.device} model={args.model} compute={compute_type}", flush=True)
    ...
    print(f"[transcriber] {len(todo)} segments to transcribe ({len(done)} already done)", flush=True)
    if not todo:
    ...
    align_model, align_metadata = whisperx.load_align_model(language_code="en", device=args.device)
    ...
    with open(args.output, "a") as out_f:
        for i, entry in enumerate(todo):
            try:
                result = transcribe_segment(model, align_model, align_metadata, entry, args.device, args.batch_size)
                record = {
                ...
                print(f"[transcriber] SKIP {entry['segmentId']}: {e}", file=sys.stderr, flush=True)
                failed += 1

            if (i + 1) % 100 == 0 or i == len(todo) - 1:
                print(f"[transcriber] {i + 1}/{len(todo)} — {ok} OK / {failed} failed", flush=True)
