...
export async function runCleanup(transcribed) {
  ...
  if (!todo.length) {
    log.success("All segments already cleaned");
    return;
  }
  ...
  await Promise.all(
    todo.map((entry, i) =>
      limit(async () => {
        try {
          const clean_text = await withRetry(() => cleanOne(entry), {
            attempts: 4,
            baseDelay: 3000,
        ...
        if ((i + 1) % 100 === 0 || i === todo.length - 1) {
          log.info(
            `Cleanup progress: ${i + 1}/${todo.length} — ${ok} OK / ${failed} failed`,
          );
        }
      }),
    ),
